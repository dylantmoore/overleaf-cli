# Task 9: Full Comment Thread Lifecycle

## Task Prompt

In my 'CLI Test Complete' project on main.tex: add a comment anchored at the word "Results" saying "This section needs data tables", then reply to that thread with "I'll add them next week", then resolve the thread. Show me the threads at each step.

## Capabilities Exercised

- Adding anchored comment (`add-comment --at-text`)
- Replying to thread (`comment`)
- Resolving thread (`resolve-thread` — needs doc ID)
- Thread listing (`threads`)
- Chaining IDs across commands

## Reference Files

- Skill body: Gotchas section (resolve-thread needs doc ID)
